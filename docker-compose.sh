...
docker-compose up -d #启动nsq 所有组件 --force-recreate 强制重新创建容器
...
docker-compose config 检查当前目录中docker-compose.yaml 文件格式是否正确
docker-compose config -f aa.yaml 检查指定文件的格式
...
docker-compose 配置项:
services 的网络隔离 networks:
services 启动顺序 depends_on:
services 的数据卷 volumes: - 宿主机目录：容器目录
services 的日志驱动   logging:
